...
Assume the standard Simons--de Weger/Hercher reduction for a nontrivial Collatz `m`-cycle and the lower-bound stages encoded in `make_case(96)` of `src/m96/affine_ladder_prefix.cpp`:
...
For any fixed `k1=j`, `src/m96/affine_ladder_prefix.cpp`, compiled and run as `affine_ladder_prefix 96 "j" ...`, enumerates every downstream valuation pattern allowed by the reduced model, including all legal values of `k2,...,k7` satisfying the internal caps and all exact affine-ladder congruence constraints.
