...
def ln_bounds(x, terms=400):
    x=F(x); z=(x-1)/(x+1); z2=z*z; s=F(0); zp=z
    for j in range(terms): s+=zp/(2*j+1); zp*=z2
    lo=2*s; tail=2*(z**(2*terms+1))/((2*terms+1)*(1-z2)); return lo, lo+tail
ln2L,ln2U=ln_bounds(2); ln3L,ln3U=ln_bounds(3)
gL=ln3L/ln2U; gU=ln3U/ln2L            # log2(3) in [gL,gU], width ~1e-200
print(f"[log2 3] bracket width ~ {float(gU-gL):.2e};  {float(gL):.12f} < log2 3 < {float(gU):.12f}")

X=1<<71
ok=True
def chk(name,cond):
    global ok; ok=ok and cond; print(f"  [{'OK' if cond else 'FAIL'}] {name}")
...
chk("log2(29*2^71+1) < 76", F(29)*X+1 < (1<<76))   # so log2(n1+1) < 76 used in caps
...
kcap=[75,120,191,303,481,763,1210]   # the i-th cap is kcap[i-1] (engine pads index 0 with a dummy)
a=F(317,200)
for i in range(1,8):
    bound = a**(i-1) * Lmax                              # k_i < bound (strict, over-estimate of truth)
    tmax = -((-bound.numerator)//bound.denominator) - 1  # ceil(bound)-1 = largest int strictly < bound
    chk(f"kcap[{i}]={kcap[i-1]} >= true-max {tmax}  (k_{i} < (317/200)^{i-1}*76 = {float(bound):.2f})", kcap[i-1] >= tmax)
...
AL_LO=F(83130157078217,52449289519716); AL_HI=F(18340740190704,11571718688839)
...
w=AL_HI-AL_LO
chk("bracket width * FRONTIER < 1 (floors cannot straddle an integer below frontier)", FRONTIER*w < 1)
chk("FRONTIER = 72057431991 (Paper 2 first positive surplus Q21+Q22)", FRONTIER==72057431991)
...
print(f"    extra = [0, 2^71, 2^71, 2^71, 2^71, 2^71, 3*2^74, 7*2^117, ceil(93*2^189/50)]")
print(f"    log2 of stages 6,7,8 ~ {76.585:.1f}? -> {float(__import__('math').log2(extra[6])):.2f}, {float(__import__('math').log2(extra[7])):.2f}, {float(__import__('math').log2(extra[8])):.2f}")
