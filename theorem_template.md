...
3. The lower-bound stage inequalities audited in `audit_lower_bound.py` or in a replacement formal proof.
4. The branch-cover lemmas in `branch_cover_theorem.md`.

Let `C96` be a certificate directory containing:

- `tasks.jsonl`, generated from `cover_m96_k1_1_75`;
- one `.log` and one `.meta.json` file for every task;
- the exact source file `affine_ladder_prefix.cpp` with matching SHA-256 hash.

If `verify_certificate.py` accepts `C96`, then no nontrivial Collatz `m=96`-cycle exists.

## Proof

Suppose a nontrivial Collatz `m=96`-cycle exists. By the imported reduction and branch-cover theorem, it belongs to exactly one branch `k1=j`, `1 <= j <= 75`, and is represented in the corresponding task of `tasks.jsonl`.
